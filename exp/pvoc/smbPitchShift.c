...
int main(int argc, char *argv[])
{
    ...
    snd_in = sf_open("clar.wav", SFM_READ, &in_info);
    ...
    snd_out = sf_open("new.wav", SFM_WRITE, &out_info);
    if( snd_out == NULL )
    {
    ...
    signal_size = in_info.frames;
    orig_signal = (float *) malloc(sizeof(float) * signal_size);
    new_signal  = (float *) malloc(sizeof(float) * signal_size);

    sf_read_float(snd_in, orig_signal, signal_size);

    smbPitchShift(SCALE,
                  signal_size,
                  32, 
                  4, 
                  (float)in_info.samplerate, 
                  orig_signal, new_signal);

    sf_write_float(snd_out, new_signal, signal_size);
